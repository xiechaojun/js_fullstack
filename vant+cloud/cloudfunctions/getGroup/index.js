...
exports.main = async (event, context) => {
  ...
  let groupList = await db.collection('user-group').where({  //collection 创建集合
    userId:openId
  })
  .get()
  let returnResult = []
  for (let item of groupList.data){
    ...
    if (oneGroup.data.length>0){
      ...
      returnReult.push(oneGroup.data[0])

    }
  }  
  return returnReult.sort((a,b) => a.createTime < b.createTime ? 1 : -1)
}