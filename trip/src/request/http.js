...
axios.interceptors.request.use(
  config => {
    ...
    // token && (config.header.Authorization = token) // token存在并且没有过期
...
var request = (options) => {
  ...
  return axios.request({
    ...
    method:options.method,  //get post 等
    data:options.body,
    ...
    throw err //请求成功，但是返回出来的东西是错的
  }).catch((error) => {
    Toast.failed('请求失败')  //请求失败
    throw error
  })
}
// http请求方式
...
export default function plugin (Vue) {
  ...
  Object.defineProperties(Vue.prototype,{  //defineProperties直接在一个对象上新增属性或者修改原有属性并返回最新的对象
    $http:{
      get() {
        const obj = {
          get:http['get'],
